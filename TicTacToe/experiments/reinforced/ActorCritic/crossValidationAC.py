...
from TicTacToe.experiments.reinforced.ActorCritic.trainActorCriticPlayer import TrainActorCriticPlayer
...
class ReinforcedCrossValidation(Experiment):
    ...
    def run(self, iterations, lr_lower_boundary, lr_upper_boundary):
        ...
        for i in range(iterations):
            ...
            self.nested_experiment.reset().run(lr=LR, steps=self.batch_size)
...
if __name__ == '__main__':
    ...
    experiment = ReinforcedCrossValidation(TrainActorCriticPlayer(games=GAMES, evaluations=EVALUATIONS, pretrained_player=PLAYER), BATCH_SIZE)
